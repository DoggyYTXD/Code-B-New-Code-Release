...
class Awards {
	public static var achievementsStuff:Array<Dynamic> = [ //Name, Description, Achievement save tag, Hidden achievement
		["JODETE SENPAI",	            "Completa Rosesb en Master",			            'Rosesb',	            false],
		["SIN ARMAS MUCHACHOS",		    "Completa Gunsb en Master",				            'Gunsb',			    false],
		["LEMON RAP",				    "Completa Southb en Master",				        'Southb',			    false],
		["SPOOKY REMIX",			    "Completa South Erect Benja Remix en Master",		'SouthR',			    false],
		["?",					        "?",				                                'MASTER',			     true],
		["?",			                "?",				                                'HARD',			         true],
		["BOMBA HUMANA???",				"Completa UnderGroundb en Master",				    'UnderGroundB',			false],
		["SOY UN MERCENARIO",	        "Completa Picob en Master",	                        'Picob',				false],
		["MICROFONO PULIDO",			"Completa el Tutorial en Master",			        'Mic',			false],
		["HUMILLACION DE 10M VISTAS",	"Completa Talentless Yt en Master",					'humilde',			    false],
	];
	public static var achievementsMap:Map<String, Bool> = new Map<String, Bool>();

	public static var henchmenDeath:Int = 0;
	public static function unlockAchievement(name:String):Void {
		FlxG.log.add('Desbloqueaste"' + name +'"');
...
class AttachedAchievement extends FlxSprite {
	...
	public function reloadAchievementImage() {
		if(Awards.isAchievementUnlocked(tag)) {
			loadGraphic(Paths.image('awards/achievementgrid'), true, 150, 150);
			animation.add('icon', [Awards.getAchievementIndex(tag)], 0, false, false);
			animation.play('icon');
		} else {
...
class AchievementObject extends FlxSpriteGroup {
	...
	public function new(name:String, ?camera:FlxCamera = null)
	{
		...
		var id:Int = Awards.getAchievementIndex(name);

		var achievementBG:FlxSprite = new FlxSprite(60, 50).makeGraphic(420, 120, FlxColor.BLACK);
		achievementBG.scrollFactor.set();

		var achievementIcon:FlxSprite = new FlxSprite(achievementBG.x + 10, achievementBG.y + 10).loadGraphic(Paths.image('awards/achievementgrid'), true, 150, 150);
		...
		var achievementName:FlxText = new FlxText(achievementIcon.x + achievementIcon.width + 20, achievementIcon.y + 16, 280, Awards.achievementsStuff[id][0], 16);
		achievementName.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT);
		achievementName.scrollFactor.set();

		var achievementText:FlxText = new FlxText(achievementName.x, achievementName.y + 32, 280, Awards.achievementsStuff[id][1], 16);
