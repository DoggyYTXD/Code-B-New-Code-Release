...
class ModchartState 
{
	...
	public var chars:Map<String, Character> = [PlayState.SONG.player2 => new Character( 100, 100, PlayState.SONG.player2)];
	public var bfs:Map<String, Boyfriend> = [PlayState.SONG.player1 => new Boyfriend( 770, 450, PlayState.SONG.player1)];
	public var gfs:Map<String, Character> = [PlayState.gf.curCharacter => new Character(400, 130, PlayState.gf.curCharacter)];
    ...
    function new()
    {
				...
				Lua_helper.add_callback(lua,"loadCharacter", function (personaje:String, x:Float, y:Float, ? isPlayer: Bool = false) {
					if (isPlayer){
                       var bf = new Boyfriend (x,y,personaje);
					   bfs  [personaje] = bf;
					}else{ 
					   var char = new Character (x,y,personaje);
					   chars [personaje] = char;
					}
					return personaje;
				});
				...
				Lua_helper.add_callback(lua, "changeNoteStyle", function(estilo:String, ?modo:Int = 0){
					PlayState.instance.changeStyle(estilo, modo);
				});
